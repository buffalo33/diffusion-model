...
def kaiming_uniform_(tensor, gain=1., mode='fan_in'):
    ...
    var = gain / max(1., fan)
    bound = np.sqrt(3.0 * var)  # Calculate uniform bounds from standard deviation
    with torch.no_grad():
...
def conv2d(in_planes, out_planes, kernel_size=(3, 3), stride=1, dilation=1, padding=1, bias=True, padding_mode='zeros',
           init_scale=1.):
    conv = nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation,bias=bias, padding_mode=padding_mode)
